...
export default function Vehicles() {
  ...
  const filtered = vehicles.filter((v) => {
    const hay = `${v.plaque} ${v.marque} ${v.modele} ${v.vin || ""} ${v.base || ""} ${v.groupe || ""}`.toLowerCase();
          ...
          <table className="w-full min-w-[860px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-left">
                {["Véhicule", "Affectation / km", "Leasing", "Assurance", "Contrôle", ""].map((h) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading && (
                <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-400">Chargement…</td></tr>
              )}
              {!isLoading && filtered.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-400">Aucun véhicule trouvé.</td></tr>
              )}
              {filtered.map((v) => {
                const m = v.metrics || {};
                const thumb = vehicleThumbSrc(v);
                return (
                  <tr key={v.id} className="group cursor-pointer transition-colors hover:bg-slate-50" data-testid={`vehicle-row-${v.id}`} onClick={() => openVehicle(v.id)}>
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-3">
                        <div className="h-12 w-[76px] shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-100 shadow-sm">
                          {thumb ? (
                            <img src={thumb} alt={v.plaque} loading="lazy"
                                 onError={(e) => { e.currentTarget.style.display = "none"; e.currentTarget.nextElementSibling.style.display = "flex"; }}
                                 className="h-full w-full object-cover" />
                          ) : null}
                          <div className="h-full w-full items-center justify-center text-slate-300" style={{ display: thumb ? "none" : "flex" }}>
                            <Truck className="h-5 w-5" />
                          </div>
                        </div>
                        <div>
                          <button
                            onClick={(e) => { e.stopPropagation(); openVehicle(v.id); }}
                            data-testid={`vehicle-plate-link-${v.id}`}
                            className="font-display text-[15px] font-bold tracking-tight text-slate-900 transition-colors group-hover:text-slate-700"
                          >
                            {v.plaque}
                          </button>
                    ...
                    <td className="px-4 py-2.5">
                      <p className="text-sm font-semibold text-slate-700">{fmtKm(v.kilometrage)}</p>
                      <p className="text-xs text-slate-400">{[v.base, v.groupe].filter(Boolean).join(" · ") || "—"}</p>
                    </td>
                    <td className="px-4 py-2.5"><StatusBadge level={m.leasing?.level} days={m.leasing?.days_remaining} showDays /></td>
                    <td className="px-4 py-2.5"><StatusBadge level={m.assurance?.level} days={m.assurance?.days_remaining} showDays /></td>
                    <td className="px-4 py-2.5"><StatusBadge level={m.controle?.level} days={m.controle?.days_remaining} showDays /></td>
                    <td className="px-4 py-2.5 text-right">
                      <button
                        onClick={(e) => { e.stopPropagation(); openVehicle(v.id); }}
                        data-testid={`open-vehicle-${v.id}`}
