...
class LlmVisionProvider(DocumentExtractionProvider):
    ...
    async def suggest_co2(self, vehicle: dict) -> dict:
        """Estimation IA du CO₂ officiel combiné (homologation constructeur, g/km).
        Retourne value_g_km=None si aucune estimation fiable. JAMAIS écrit sans validation humaine."""
        if not self.api_key:
            raise RuntimeError("Aucune clé d'extraction configurée (ANTHROPIC_API_KEY ou EMERGENT_LLM_KEY)")
        from emergentintegrations.llm.chat import LlmChat, UserMessage
        cg = vehicle.get("carte_grise") or {}
        ...
        prompt = (
            "Quelle est l'émission de CO₂ officielle combinée (homologation constructeur, en g/km) de ce véhicule ?\n"
            f"Véhicule : {desc}\n\n"
            "Règles STRICTES :\n"
            "- Réponds UNIQUEMENT en JSON strict : {\"value_g_km\": <nombre|null>, \"norme\": \"WLTP\"|\"NEDC\"|null, "
            "\"confidence\": <0..1>, \"rationale\": \"<justification courte en français>\"}\n"
            "- value_g_km = CO₂ combiné d'homologation pour ce modèle/génération/motorisation (WLTP de préférence, sinon NEDC — indique la norme).\n"
            "- Véhicule 100% électrique : value_g_km = 0.\n"
            "- Si plusieurs variantes existent, prends la valeur de la variante la plus courante et dis-le dans rationale.\n"
            "- Si tu n'es pas raisonnablement sûr : value_g_km = null.\n"
            "- N'invente JAMAIS une valeur farfelue ; plage plausible 0–500 g/km."
        )
        chat = LlmChat(
            api_key=self.api_key,
            session_id=f"co2-{uuid.uuid4()}",
            system_message="Tu es un expert des fiches techniques constructeur automobile. Tu réponds uniquement en JSON strict.",
        ).with_model(self.llm_provider, self.model)
        resp = await chat.send_message(UserMessage(text=prompt))
        text = resp if isinstance(resp, str) else getattr(resp, "text", str(resp))
        ...
        try:
            ...
            conf = None
        return {"value_g_km": value, "norme": norme, "confidence": conf,
                "rationale": (parsed.get("rationale") or "").strip() or None}
